...
/* The height of the window's title bar is 30 pixels */
/* My intention was for the window to be completely square */
constexpr int ScreenWidth = 800;
constexpr int ScreenHeight = 770;
// #define FULLSCREEN
...
static const char* TemplateVersion = "Coal Critters";
...
public:
	...
	/* Added an operator overload for multiplying by an integer */
	vec2 operator * ( int operand ) const { return vec2( x * static_cast<float>(operand), y * static_cast<float>(operand) ); }
	/* Added an operator overload for dividing a vec2 by a float */
	vec2 operator / ( float operand ) const { return vec2( x / operand, y / operand ); }
	...
	/* Added an operator overload for dividing a vec2 by a float */
	void operator /= ( float a ) { x /= a; y /= a; }
	/* Added an operator overload for dividing a vec2 by an integer */
	void operator /= ( int a ) { x /= static_cast<float>(a); y /= static_cast<float>(a); }
	float& operator [] ( const int idx ) { return cell[idx]; }
	float length() { return sqrtf( x * x + y * y ); }
	float sqrLentgh() { return x * x + y * y; }
	/* Added a way of calculating the distance between two vec2's */
	float dist( vec2 a ) { return sqrtf( (x-a.x)*(x-a.x) + (y-a.y)*(y-a.y) ); }
	/* Added 2 functions for the distance in a specific axis */
	float distToPointX( float x2 ) { return (x2 - x); }
	float distToPointY( float y2 ) { return (y2 - y); }
	vec2 normalized() { float r = 1.0f / length(); return vec2( x * r, y * r ); }
	/* Changed so the radius can now be set when the function is called */
	/* Also now prevents dividing by 0 */
	void normalize( float radius = 1.0f )
	{
		if (length() != 0.0f) { float r = radius / length(); x *= r; y *= r; }
	}
